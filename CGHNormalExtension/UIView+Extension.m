...
+ (CGFloat)widthWithString:(NSString *)string fontsize:(CGFloat)fontsize{
    CGRect rect = [string boundingRectWithSize:CGSizeMake(10000, SCREEN_HEIGHT) options:NSStringDrawingTruncatesLastVisibleLine |NSStringDrawingUsesLineFragmentOrigin |NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : [self adjustFont:fontsize]} context:nil];
    
    return rect.size.width;
}

+ (void)removeTheViewControllerFromTheNavigationVCs:(UIViewController *)vc{
    NSMutableArray *arr = [NSMutableArray arrayWithArray:vc.navigationController.viewControllers];
    [arr removeObject:vc];
